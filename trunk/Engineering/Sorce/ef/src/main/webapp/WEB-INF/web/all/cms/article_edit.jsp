...
<title>文章操作 - ${article.title }</title>
...
<body> <shiro:hasRole name="admin">
  ...
  <div class="J_content">
	...
	<script type="text/javascript" src="<%=path %>/ueditor/editor_all.js"></script>
	<script src="<%=path %>/static/js/uploadify/jquery.uploadify.js"></script>
	<script type="text/javascript">
				...
				valid : function(form){
					$('#editForm').ajaxSubmit({
						success : function(result){
							if(result.success){
								layer.alert('保存成功');
								window.location.href="<%=path%>/cmsController/${user.role.name}/article/list.do"
							}
						}
					});
				}
       ...
       function myBack(){
    	   window.location.href="<%=path%>/cmsController/${user.role.name}/article/list.do?type=tongguo";
       }
