...
<div class="col-sm-10 col-sm-offset-2 col-md-10 col-md-offset-2 col-lg-10 col-lg-offset-2">
 ...
 <form action="<%=path%>/role/addResources.do" method="post">
           <div class="input-group">
           <input type="hidden" name="roleId" value="${role.id }"/>
  <span class="input-group-addon" id="basic-addon1">角色名</span>
  <input type="text" name="name" class="form-control" placeholder="name" aria-describedby="basic-addon1" value="${role.name }">
</div>
<div class="input-group">
  <span class="input-group-addon" id="basic-addon1">URL(url之间以';'分割)</span>
  <input type="text" name="resources" class="form-control" placeholder="name" aria-describedby="basic-addon1"  value=' <c:forEach items="${sets }" var="s">${s.url };</c:forEach>'>
</div>
<div class="input-group">
