					...
					<div class="i_menu">
						<div data-ui="profile" class="J_profile">
							<div class="p_user">
								...
								<div class="u_name">
									<span>${sessionScope.user.sno }222</span>
								</div>
							</div>
							<div class="p_list">
								<div class="l_item">
		...
		<div class="h_left">
			<div class="l_nav">
                ...
                ">
					<div class="i_default">
						<a
							href="<%=path %>/cmsController/admin/article/list.do?type=zhiding&rootColumnId=${articleQueryDTO.rootColumnId}"><span>置顶文章</span><b></b></a>
					</div>
				</div>
				<div
					class="n_item
                ...
                ">
					<div class="i_default">
						<a
							href="<%=path %>/cmsController/admin/article/list.do?type=shenhe&rootColumnId=${articleQueryDTO.rootColumnId}"><span>审核列表</span><b></b></a>
					</div>
				</div>
			</div>
		</div>
		...
		<div class="mt20 plr20">
			<form action="<%=path %>/cmsController/admin/article/list.do" id="queryForm"
				...
				<input type="hidden" name="type" value="${type }" /> <input
					type="hidden" name="columnId" value="${articleQueryDTO.columnId }" />
				<input type="hidden" name="rootColumnId"
					value="${articleQueryDTO.rootColumnId }" />
									...
									<tbody>
										<c:forEach items="${page.list }" var="p">
											<tr>
												...
												<td>
													<div class="t_text tc">${p.columnInfo.name }</div>
												</td>
												<td>
												...
												<td>
													<div class="t_link">
														<a href="javascript:myEdit('${p.id }');"><i
															class="icon"></i>编辑</a> <a
															href="<%=path%>/cmsController/article/ajax/delete.do?ids=${p.id}"><i
															class="icon"></i>删除</a>

														<c:choose>
															<c:when test="${p.isAudit eq 'true' }">
																<a
																	href="<%=path%>/cmsController/admin/article/ajax/audit.do?ids=${p.id}&auditFlag=0"><i
																	class="icon"></i>审核不通过</a>

															</c:when>
															<c:otherwise>
																<font color="red"> <a
																	href="<%=path%>/cmsController/admin/article/ajax/audit.do?ids=${p.id}&auditFlag=1"><i
																		class="icon"></i>审核通过</a>
																</font>
															</c:otherwise>
														</c:choose>
														<c:choose>
															<c:when test="${p.isTop eq 'true' }">
																<font color="red"> <a
																	href="<%=path%>/cmsController/article/ajax/top.do?ids=${p.id}&topFlag=0"><i
																	class="icon"></i>取消置顶</a>
																	</font>
															</c:when>
															<c:otherwise>
																<font color="red"> <a
																	href="<%=path%>/cmsController/article/ajax/top.do?ids=${p.id}&topFlag=1"><i
																		class="icon"></i>置顶</a>
																</font>
															</c:otherwise>
														</c:choose>
...
<%-- 
		function updDeleteFlag(id, deleteFlag){
			...
			layer.confirm('确定要' + msg + '记录吗？', function(index){
				...
				$.ajax({
					url : '<%=path%>/cms/article/ajax/delete',
					type : 'post',
					data : {
						'ids' : ids,
		...
		function myEdit(id){
	    	...
	    	window.location.href='<%=path%>/cmsController/article/edit.do?id='+id+'&rootColumnId='+rootColumnId+"&columnId="+columnId;
		}
